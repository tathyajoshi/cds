...
- `shadow`: React Native shadow styles. A token is authored in one of two forms — never both.

  The cross-platform `boxShadow` form, which renders on iOS and Android:
  ...
  The legacy `shadow*` form, which only renders on iOS. This is what the built-in themes ship:
  ...
  `boxShadow` requires React Native 0.76+ on the New Architecture. Only the array form is accepted;
  the CSS string shorthand (`'0px 8px 24px rgba(0,0,0,0.12)'`) is not.
...
4. Shadow values are provided as React Native shadow style objects. Prefer the cross-platform `boxShadow` form; the `shadowColor`/`shadowOffset`/`shadowOpacity`/`shadowRadius` props are iOS-only and are deprecated.
